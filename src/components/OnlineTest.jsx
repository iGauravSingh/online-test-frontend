...
//    *    EFFECTS
//    *********************/
//   // Fetch tests when available tests step (step 2) is reached.
//   useEffect(() => {
//     if (step === 2) {
//       fetch("http://localhost:3000/tests/all")
...
//           <div className="max-w-xl mx-auto bg-white shadow-md rounded p-6">
//             <h2 className="text-2xl font-bold mb-4 text-[#000000]">Available Tests</h2>
//             {tests.filter((test) => test.isLive).length === 0 ? (
//               <p className="text-gray-700 mb-4">No test for now</p>
//             ) : (
//               tests
//                 .filter((test) => test.isLive)
//                 .map((test) => (
//                   <div key={test.id} className="flex items-center justify-between mb-4 p-4 border rounded">
...
//           <div className="bg-white shadow-md rounded p-6">
//             <div className="flex justify-between items-center mb-4">
//               <h2 className="text-xl font-bold text-[#000000]">{selectedTest.title}</h2>
//               <div className="flex items-center text-[#004aad] font-semibold">
//                 <FaClock className="mr-2" />
//                 <span>{formatTime(timeLeft)}</span>
//               </div>
...
//                     key={idx}
//                     className={`block p-2 mb-2 border rounded cursor-pointer hover:bg-blue-100 ${
//                       answers[selectedTest.questions[currentQuestionIndex].id] === option.text
//                         ? "bg-blue-200 border-blue-400"
//                         : "bg-white border-gray-300"
//                     }`}
//                   >
...
//                       }
//                       onChange={() =>
//                         handleOptionSelect(selectedTest.questions[currentQuestionIndex].id, option.text)
//                       }
//                       className="mr-2"
//                     />
//                     {option.text}
...
//             <h2 className="text-2xl font-bold mb-4 text-[#000000]">Available Tests</h2>
//             {loadingTests ? (
//               <p className="text-gray-700 mb-4">Loading tests...</p>
//             ) : tests.filter((test) => test.isLive).length === 0 ? (
//               <p className="text-gray-700 mb-4">No test for now</p>
//             ) : (
//               tests
...
export default function OnlineTest() {
  ...
  // Calculate score by comparing user answers with the correct answer
  ...
  const handleSubmitTest = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/tests/${selectedTest.id}/submit-result`,
        {
          method: "POST",
      ...
      if (!response.ok) {
        console.error("Error submitting test result:", response.statusText);
      } else {
