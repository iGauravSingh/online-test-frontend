...
const Dashboard = () => {
  ...
  // Tests fetched from backend
  const [tests, setTests] = useState([]);

  // Candidate results fetched from backend for a selected test
  ...
  // Fetch candidate results when a test is selected and showResult is true
  useEffect(() => {
  ...
  const fetchCandidateResults = async () => {
    const token = localStorage.getItem("access_token");
    try {
      // Assuming the endpoint is dynamic based on selectedTest.id
      const response = await fetch(`http://localhost:3000/tests/${selectedTest.id}/results`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      if (response.ok) {
        ...
        alert(`Error fetching candidate results: ${data.message || "Something went wrong"}`);
      }
    } catch (error) {
      console.error(error);
      alert(`Error: ${error.message}`);
  ...
  const handleAddQuestion = (e) => {
    ...
    const newQuestion = {
      questionText,
      imageUrl: "", // default value; update if an image is provided
      options: [
        { text: optionA, isCorrect: correctOptions.a },
        { text: optionB, isCorrect: correctOptions.b },
        { text: optionC, isCorrect: correctOptions.c },
  ...
  const handleCreateTest = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        alert(data.message);
        // Optionally update local tests or refetch from backend
        setTests([...tests, newTest]);
        setTestName("");
        setDuration("");
        setLiveStatus(false);
          ...
          <div className="p-4">
            ...
            <div className="overflow-x-auto">
              {candidateResults.length === 0 ? (
                <p>No candidate results found.</p>
              ) : (
                <table className="min-w-full bg-white border">
                  <thead>
                    <tr>
                  ...
                  <tbody>
                    {candidateResults?.map((candidate, idx) => (
                      ...
                      <tr key={idx}>
                        ...
                        <td className="py-2 px-4 border">{candidate.userName}</td>
                        <td className="py-2 px-4 border">{candidate.userPhone}</td>
                        <td className="py-2 px-4 border">{candidate.userEmail}</td>
                        <td className="py-2 px-4 border">{candidate.score}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
